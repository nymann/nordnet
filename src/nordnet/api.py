...
class NordnetApi:
    def __init__(self, username: str, password: str) -> None:
        self.session = NordnetSession(username=username, password=password)
        self.helper = HttpJsonHelper(session=self.session)

    def accounts(self) -> list[Account]:
        accounts_response = self.helper.get("/api/2/accounts")
    ...
    def accounts_info(self, accounts: list[Account]) -> list[AccountInfo]:
        ...
        account_info_response = self.helper.get(
